# Streamlit frontend for Job Finder app
...
# Step 2: Skills input and analysis
    ...
    if skills_domain.empty:
        ...
        question = st.session_state["Questions"].query("Domain == @domain")["Questions"].iloc[0]
...
# Step 3: Display results
    ...
    for i, (_, row) in enumerate(top3.iterrows()):
        with cols[i]:
            st.metric(label=f"{medals[i]} #{i+1}", value=row["Job"], delta=f"Score: {row['Score']:.3f}")
    ...
    for idx, (_, row) in radar_data.iterrows():
    ...
    with st.expander("📋 Detailed Competency Scores"):
        st.dataframe(st.session_state["Skills_competency"], use_container_width=True)
